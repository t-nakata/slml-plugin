 ...
 */
function extractDCUIBlocks(markdown) {
...
function parseDCUI(dcui) {
    ...
    if (lines.length > 0) {
        ...
        if (firstLine.startsWith('#')) {
            ...
            if (widthMatch)
                screen.width = parseInt(widthMatch[1], 10);
            if (heightMatch)
                screen.height = parseInt(heightMatch[1], 10);
            if (bgColorMatch)
                screen.backgroundColor = bgColorMatch[1];
            lines.shift();
        }
    }
    // Parse elements
...
export { parseDCUI, processMarkdown };
