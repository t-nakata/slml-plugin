 ...
 *
 * @param slmlCode - The SLML code block content
 * @returns Parsed SLML screen object
 */
export function parseSLML(slmlCode: string): SLMLScreen {
    ...
    // Parse elements
    const elements: SLMLElement[] = [];
    const parentStack: { element: SLMLElement, indentLevel: number }[] = [];
            ...
            if (element) {
                ...
                // If this is at the same level as the last element
                    ...
                    if (parentStack.length > 0 && indentLevel > 0) {
                        const parent = parentStack[parentStack.length - 2]?.element;
    ...
    console.log(`Parsed SLML Screen: ${title}, Elements: ${elements.length}, Width: ${width}, Height: ${height}, Background Color: ${backgroundColor}`);
 ...
 *
 * @param elementText - The element text (e.g., "Input: Username")
 * @returns Parsed SLML element object
 */
function parseElement(elementText: string): SLMLElement | null {
    ...
    let children: SLMLElement[] | undefined;
 ...
 * @returns Array of extracted SLML code blocks
 */
export function extractSLMLFromMarkdown(markdownContent: string): string[] {
    const slmlBlocks: string[] = [];
 ...
 *
 * @param markdownContent - The Markdown content
 * @returns Array of parsed SLML screen objects
 */
export function processMarkdown(markdownContent: string): SLMLScreen[] {
